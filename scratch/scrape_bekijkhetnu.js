/**
 * OSIRIS — harvester for bekijkhet.nu (throwaway, like scrape_germany.js).
 *
 * bekijkhet.nu is a link directory, not a feed source: every entry points at the
 * operator's own landing page, over 383 hosts, with no coordinates and no direct
 * stream URLs. This script turns the harvestable part of it into a generated
 * CctvCamera module.
 *
 * Three stages, run in order:
 *
 *   node scratch/scrape_bekijkhetnu.js harvest   -> bekijkhetnu-harvest.json
 *   node scratch/scrape_bekijkhetnu.js geocode   -> bekijkhetnu-places.json
 *   node scratch/scrape_bekijkhetnu.js emit      -> ../src/app/api/cctv/bekijkhetnu.generated.ts
 *
...
const HARVEST = path.join(HERE, 'bekijkhetnu-harvest.json');
const PLACES = path.join(HERE, 'bekijkhetnu-places.json');
const OUT = path.join(HERE, '..', 'src', 'app', 'api', 'cctv', 'bekijkhetnu.generated.ts');
...
/** bekijkhet.nu serves raw entities; strip the handful its editors actually use. */
...
async function harvest() {
  ...
  for (const [region, page] of PAGES) {
    const html = await fetchText(`https://www.bekijkhet.nu/${page}`);
 ...
 *   'external' — the stream is real and live, but locked to the operator's own
 *                page by an expiring token or a Referer check. A link out is
 *                the honest option; copying it here would ship a dead URL.
 *
 * Anything else is a long-tail page we cannot read, and returns null.
 */
function playableKind(row) {
  const url = row.url;
  if (row.hls) return 'hls';
...
function build() {
  ...
  const cams = [];
  const ids = new Set();
  const dropped = { unreadable: 0, unplaced: [], noPlan: 0 };
  ...
  for (const row of rows) {
    ...
    let id = `bkh-${slug(plan.country)}-${slug(plan.city)}-${slug(row.desc)}`;
    for (let n = 2; ids.has(id); n++) id = `bkh-${slug(plan.country)}-${slug(plan.city)}-${slug(row.desc)}-${n}`;
    ids.add(id);
...
function emit() {
  ...
  const blocks = Object.entries(buckets).map(([name, list]) => {
    ...
    return `export const ${name}_BEKIJKHET_CAMERAS: CctvCamera[] = [\n${lines.join('\n')}\n];`;
  });

  const header = `import type { CctvCamera } from './types';

/**
 * OSIRIS — public webcams indexed by bekijkhet.nu (GENERATED).
 *
 ...
 * feeds — no coordinates, no stream URLs, ${'383'} different hosts. This file is
 * what is left after reading those pages, and only what OSIRIS can actually
 * show. Regenerate with scratch/scrape_bekijkhetnu.js.
 *
 * The ${'26'} Rijkswaterstaat motorway cameras the index also lists are skipped:
 * netherlands.ts already fetches those live from the RWS API.
 *
 * Four shapes come out of it:
 *
 *   HLS      — the webcam-*.nl Wowza platform, where the playlist carries no
 *              token. streamlock.net sends Access-Control-Allow-Origin: *, so
 *              these play straight from the browser and need no proxy entry.
 ...
 *   iframe   — a provider embed page that stands on its own.
 *   link     — the stream is live but locked to the operator's page, either by
 *              a Wowza token that expires inside twenty minutes or by a Referer
 *              check the browser cannot satisfy from here. These carry
 *              external_url alone, which puts the viewer into its external
 *              state — a way through to the camera rather than a copy of it.
 *              That is also how bekijkhet.nu prefers to be read: they link to
 *              the operator's whole site so people see whose camera it is.
 *
 * external_url always points at the operator, not at bekijkhet.nu.
 *
 * Coordinates were resolved via OpenStreetMap Nominatim, country-filtered, and
 * cached in scratch/bekijkhetnu-places.json so regenerating does not ask again.
 * Dutch cameras are looked up on their caption first ("Sint Servaasbrug,
 * Maastricht") and fall back to the village. Cameras whose caption names no
 * fixed place — aggregator channels cycling through dozens of locations — are
 * left out rather than pinned somewhere they are not.
 *
...
`;
  ...
  console.error(`\n  overgeslagen: ${dropped.unreadable} niet uit te lezen, ${dropped.noPlan} zonder vaste plaats`);
...
if (require.main === module) {
  ...
  if (!stages[stage]) {
    console.error('gebruik: node scratch/scrape_bekijkhetnu.js <harvest|geocode|emit>');
    process.exit(1);
  }
