...
function getRiskLabel(score: number): string {
  if (score >= 8) return 'risk.critical';
  if (score >= 6) return 'risk.high';
  if (score >= 4) return 'risk.elevated';
  return 'risk.low';
}

function timeAgo(dateStr: string, t: (k: string, v?: Record<string, any>) => string): string {
  try {
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="hud-text text-[12px] text-[var(--text-primary)]">{t('intel.title')}</span>
          ...
          {news.some((n: any) => n.risk_score >= 8) && (
            <span className="gotham-tag gotham-tag--critical" style={{ fontSize: '7px', padding: '1px 4px' }}>{t('intel.alerts')}</span>
          )}
        </div>
        <div className="flex items-center gap-2">
