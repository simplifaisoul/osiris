          ...
          >
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="hud-text text-[10px] text-[var(--text-primary)]">{t('share.title')}</span>
              </div>
            ...
            <div className="mb-3 p-2 rounded-lg bg-[var(--bg-void)] border border-[var(--border-primary)]">
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <span className="text-[7px] font-mono text-[var(--text-muted)] tracking-widest">{t('share.currentView')}</span>
              </div>
              <div className="text-[8px] font-mono text-[var(--text-secondary)]">
                {mouseCoords ? `${mouseCoords.lat.toFixed(4)}°, ${mouseCoords.lng.toFixed(4)}°` : '—'} · {t('share.zoom')} {mapView.zoom.toFixed(1)}
              </div>
              <div className="text-[7px] font-mono text-[var(--text-muted)] mt-1">
                {t('share.layersActive', { n: Object.values(activeLayers).filter(Boolean).length })}
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <span className="text-[7px] font-mono text-[var(--text-muted)] tracking-widest">{t('share.shareableLink')}</span>
              </div>
            ...
            <div className="flex gap-2">
              <a
                href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(t('share.xPostText'))}&url=${encodeURIComponent(generateShareUrl())}`}
                target="_blank"
              ...
              <a
                href={`https://reddit.com/submit?url=${encodeURIComponent(generateShareUrl())}&title=${encodeURIComponent(t('share.redditTitle'))}`}
                target="_blank"
