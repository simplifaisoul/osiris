...
// Resolve a CSS custom property to a concrete color string. WebGL map layers
// (canvas icons, paint colors) can't read CSS variables, so we snapshot the
// active theme's accent at map-init time. Popups are real DOM and use the
// variables directly, so they re-theme live; markers pick up a new theme on
// the next map load/reload.
...
function OsirisMap({ data, activeLayers, onEntityClick, onMouseCoords, onRightClick, onViewStateChange, flyToLocation, projection = 'globe', mapStyle = 'dark', sweepData, scanTargets = [] }: OsirisMapProps) {
  ...
  useEffect(() => {
    ...
    // بررسی پشتیبانی WebGL (در بعضی محیط‌های sandbox / webview ممکن است غیرفعال باشد)
    try {
      const canvas = document.createElement('canvas');
      const gl =
      ...
      if (!gl) {
        setWebglError('webgl_unavailable');
        return;
      }
    } catch {
      setWebglError('webgl_check_failed');
      return;
    }
    ...
    try {
      ...
      setWebglError(e?.message || 'webgl_init_failed');
      return;
    }

    map.on('load', () => {
      mapRef.current = map;
      // Snapshot the active theme's accent colors for WebGL layers.
      const themeGold = cssVar('--gold-primary', '#D4AF37');
      const themeCyan = cssVar('--cyan-primary', '#00E5FF');
      // Create icons
      createIcon(map, 'plane-cyan', themeCyan, 24);
      ...
      createDot(map, 'dot-gold', themeGold, 8);
      ...
      }, paint: { 'text-color': themeGold, 'text-halo-color': '#000', 'text-halo-width': 1, 'text-opacity': 0.85 }});
      ...
      // Colour differs by kind: geolocation results are cyan, OSINT scan targets red.
      map.addLayer({ id: 'scan-targets-glow', type: 'circle', source: 'scan-targets', paint: {
        ...
        'circle-color': ['match', ['get','kind'], 'geolocate', themeCyan, '#FF3D3D'], 'circle-opacity': 0.2, 'circle-blur': 1,
      }});
      map.addLayer({ id: 'scan-targets-dots', type: 'circle', source: 'scan-targets', paint: {
        ...
        'circle-color': ['match', ['get','kind'], 'geolocate', themeCyan, '#FF3D3D'], 'circle-opacity': 0.95,
      ...
      map.addLayer({ id: 'scan-targets-label', type: 'symbol', source: 'scan-targets', layout: {
        ...
        'text-offset': [0, 2], 'text-max-width': 14, 'text-allow-overlap': false,
      }, paint: { 'text-color': ['match', ['get','kind'], 'geolocate', themeCyan, '#FF3D3D'], 'text-halo-color': '#000', 'text-halo-width': 1.5, 'text-opacity': 0.9 }});
    ...
    ['fl-commercial','fl-private','fl-jets','fl-military'].forEach(layer => {
      map.on('click', layer, e => {
        ...
        popup(coords, `<div style="${pStyle}border:1px solid rgba(var(--accent-rgb),0.3);">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;font-size:11px;">
            <div><span style="color:#5C5A54;font-size:9px;">${t('mapPopup.model')}</span><br/><span style="color:#E8E6E0;">${p.model||'—'}</span></div>
            <div><span style="color:#5C5A54;font-size:9px;">${t('mapPopup.alt')}</span><br/><span style="color:var(--cyan-primary);">${p.alt?Math.round(p.alt)+'m':'—'}</span></div>
            <div><span style="color:#5C5A54;font-size:9px;">${t('mapPopup.speed')}</span><br/><span style="color:#E8E6E0;">${p.speed_knots||'—'}kt</span></div>
            <div><span style="color:#5C5A54;font-size:9px;">${t('mapPopup.heading')}</span><br/><span style="color:#E8E6E0;">${Math.round(p.heading||0)}°</span></div>
            <div><span style="color:#5C5A54;font-size:9px;">${t('mapPopup.reg')}</span><br/><span style="color:#E8E6E0;">${p.registration||'—'}</span></div>
            <div><span style="color:#5C5A54;font-size:9px;">${t('mapPopup.pos')}</span><br/><span style="color:#E8E6E0;">${coords[1].toFixed(2)},${coords[0].toFixed(2)}</span></div>
          </div>
          <div style="margin-top:12px;display:flex;gap:6px;flex-wrap:wrap;">
            <a href="https://www.flightaware.com/live/flight/${cs}" target="_blank" style="${linkStyle}color:var(--gold-primary);border:1px solid rgba(var(--accent-rgb),0.4);background:rgba(var(--accent-rgb),0.1);">⚡ FLIGHTAWARE</a>
            <a href="https://globe.adsbexchange.com/?icao=${p.icao24||''}" target="_blank" style="${linkStyle}color:var(--cyan-primary);border:1px solid rgba(var(--accent-2-rgb),0.4);background:rgba(var(--accent-2-rgb),0.1);">📡 ADS-B</a>
    ...
    map.on('click', 'eq-circles', e => {
      ...
      popup(coords, `<div style="${pStyle}border:1px solid rgba(255,149,0,0.3);">
        <div style="color:#FF9500;font-size:14px;font-weight:700;margin-bottom:4px;">${t('mapPopup.earthquakeTitle', { m: p.magnitude })}</div>
        <div style="font-size:9px;color:#E8E6E0;margin-bottom:8px;">${p.place||t('mapPopup.unknownLocation')}</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;font-size:9px;">
          <div><span style="color:#5C5A54;">${t('mapPopup.depth')}</span><br/><span style="color:#E8E6E0;">${p.depth||'—'}km</span></div>
          <div><span style="color:#5C5A54;">${t('mapPopup.coords')}</span><br/><span style="color:#E8E6E0;">${coords[1].toFixed(3)}, ${coords[0].toFixed(3)}</span></div>
        </div>
        <a href="${p.source === 'NIGGG-BAS' ? 'https://ndc.niggg.bas.bg/' : `https://earthquake.usgs.gov/earthquakes/eventpage/${p.id||''}`}" target="_blank" style="${linkStyle}color:#FF9500;border:1px solid rgba(255,149,0,0.4);background:rgba(255,149,0,0.1);">📊 ${p.source === 'NIGGG-BAS' ? 'NIGGG-BAS' : t('mapPopup.usgsDetails')}</a>
      </div>`);
    });
    ...
    map.on('click', 'sat-dots', e => {
      ...
      const coords = (e.features[0].geometry as any).coordinates;
      popup(coords, `<div style="${pStyle}border:1px solid rgba(var(--accent-rgb),0.3);">
        <div style="color:var(--gold-primary);font-size:12px;font-weight:700;letter-spacing:0.1em;margin-bottom:4px;">🛰️ ${p.name}</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;font-size:9px;margin-bottom:8px;">
          <div><span style="color:#5C5A54;">${t('mapPopup.mission')}</span><br/><span style="color:${p.color||'#aaa'};">${p.mission||t('common.unknown')}</span></div>
          <div><span style="color:#5C5A54;">${t('mapPopup.pos')}</span><br/><span style="color:#E8E6E0;">${coords[1].toFixed(2)}°, ${coords[0].toFixed(2)}°</span></div>
        </div>
        <a href="https://www.n2yo.com/?s=${encodeURIComponent(p.name||'')}" target="_blank" style="${linkStyle}color:var(--gold-primary);border:1px solid rgba(var(--accent-rgb),0.4);background:rgba(var(--accent-rgb),0.1);">🔭 ${t('mapPopup.trackOnN2yo')}</a>
      </div>`);
    });
    ...
    map.on('click', 'fires-heat', e => {
      ...
      popup(coords, `<div style="${pStyle}border:1px solid rgba(255,107,0,0.3);">
        <div style="color:#FF6B00;font-size:12px;font-weight:700;margin-bottom:6px;">🔥 ${t('mapPopup.activeFire')}</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;font-size:9px;margin-bottom:8px;">
          <div><span style="color:#5C5A54;">${t('mapPopup.brightness')}</span><br/><span style="color:#FF6B00;">${p.brightness||'—'}K</span></div>
          <div><span style="color:#5C5A54;">${t('mapPopup.coords')}</span><br/><span style="color:#E8E6E0;">${coords[1].toFixed(3)}°, ${coords[0].toFixed(3)}°</span></div>
        </div>
        <a href="https://firms.modaps.eosdis.nasa.gov/map/#d:24hrs;l:noaa20-viirs,viirs,modis_a,modis_t;@${coords[0]},${coords[1]},10z" target="_blank" style="${linkStyle}color:#FF6B00;border:1px solid rgba(255,107,0,0.4);background:rgba(255,107,0,0.1);">🛰️ ${t('mapPopup.nasaFirmsMap')}</a>
      </div>`);
    });
    ...
    map.on('click', 'gdelt-dots', e => {
      ...
      popup(coords, `<div style="${pStyle}border:1px solid rgba(255,61,61,0.3);">
        <div style="color:#FF3D3D;font-size:12px;font-weight:700;margin-bottom:6px;">⚠️ ${t('mapPopup.conflictEvent')}</div>
        <div style="font-size:9px;color:#E8E6E0;margin-bottom:8px;line-height:1.4;">${p.name||t('mapPopup.unclassifiedIncident')}</div>
        <div style="display:flex;gap:6px;">
          ${p.url ? `<a href="${p.url}" target="_blank" style="${linkStyle}color:#FF3D3D;border:1px solid rgba(255,61,61,0.4);background:rgba(255,61,61,0.1);">${t('mapPopup.source')}</a>` : ''}
          <a href="https://www.google.com/maps/@${coords[1]},${coords[0]},12z" target="_blank" style="${linkStyle}color:#448AFF;border:1px solid rgba(68,138,255,0.4);background:rgba(68,138,255,0.1);">${t('mapPopup.map')}</a>
        </div>
      </div>`);
    });
    ...
    map.on('click', 'conflict-icons', e => {
      ...
      popup(coords, `<div style="${pStyle}border:1px solid ${color}40;">
        <div style="color:${color};font-size:12px;font-weight:700;margin-bottom:6px;">⚠️ ${p.label || t('mapPopup.warningEvent')}</div>
        <div style="font-size:10px;color:#E8E6E0;margin-bottom:8px;line-height:1.4;">${p.description || t('mapPopup.globalEventDefaultBody')}</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;font-size:9px;margin-bottom:8px;">
          <div><span style="color:#5C5A54;">${t('mapPopup.severity')}</span><br/><span style="color:${color};">${(p.severity||'unknown').toUpperCase()}</span></div>
          <div><span style="color:#5C5A54;">${t('mapPopup.coords')}</span><br/><span style="color:#E8E6E0;">${coords[1].toFixed(3)}°, ${coords[0].toFixed(3)}°</span></div>
        </div>
      </div>`);
    });
    ...
    map.on('click', 'scan-targets-dots', (e: any) => {
      ...
      popup(coords, `<div style="${pStyle}border:1px solid rgba(255,61,61,0.5);">
        <div style="color:#FF3D3D;font-size:12px;font-weight:700;margin-bottom:6px;">🎯 ${t('mapPopup.target')}: ${p.id}</div>
        <div style="font-size:9px;color:#E8E6E0;margin-bottom:8px;">${p.city || t('common.unknown')}, ${p.country || t('common.unknown')} — ${p.isp || t('mapPopup.unknownIsp')}</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;font-size:9px;">
          <div><span style="color:#5C5A54;">${t('mapPopup.type')}</span><br/><span style="color:var(--cyan-primary);">${(p.type || t('common.unknown')).toUpperCase()}</span></div>
          <div><span style="color:#5C5A54;">${t('mapPopup.coords')}</span><br/><span style="color:#E8E6E0;">${coords[1].toFixed(3)}°, ${coords[0].toFixed(3)}°</span></div>
        </div>
      </div>`);
    });
    ...
    map.on('click', 'sweep-device-dots', (e: any) => {
      ...
      popup(coords, `<div style="font-family:monospace;font-size:11px;color:#E8E6E0;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:8px;">
          <div><span style="color:#5C5A54;">${t('mapPopup.ports')}</span><br/><span style="color:#E8E6E0;">${ports.length}</span></div>
          <div><span style="color:#5C5A54;">${t('mapPopup.risk')}</span><br/><span style="color:${riskColors[p.risk_level] || '#666'};">${p.risk_level}</span></div>
        </div>
        <div style="font-size:9px;color:#8A8880;margin-bottom:6px;">${t('mapPopup.openPorts', { ports: ports.slice(0, 12).join(', ') + (ports.length > 12 ? ' ...' : '') })}</div>
        ${vulns.length > 0 ? `<div style="font-size:9px;color:#FF3D3D;margin-bottom:6px;">⚠ ${t('mapPopup.cves', { cves: vulns.slice(0, 5).join(', ') })}${vulns.length > 5 ? ` +${t('mapPopup.more', { n: (vulns.length - 5) })}` : ''}</div>` : ''}
      </div>`);
    });
    ...
    map.on('click', 'balloon-dots', e => {
      ...
      popup(coords, `<div style="${pStyle}border:1px solid ${p.color}40;">
        ...
        <div style="font-size:9px;color:#aaa;margin-bottom:8px;">${t('mapPopup.type')}: ${p.type.toUpperCase()} · ${t('mapPopup.status')}: ${p.status.toUpperCase()}</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;font-size:9px;">
          <div><span style="color:#5C5A54;">${t('mapPopup.altitude')}</span><br/><span style="color:#E8E6E0;">${p.altitude} m</span></div>
          <div><span style="color:#5C5A54;">${t('mapPopup.speed')}</span><br/><span style="color:#E8E6E0;">${Math.round(p.speed)} km/h</span></div>
          <div><span style="color:#5C5A54;">${t('mapPopup.vertRate')}</span><br/><span style="color:${p.verticalRate > 0 ? '#00E676' : '#FF3D3D'};">${p.verticalRate.toFixed(1)} m/s</span></div>
          <div><span style="color:#5C5A54;">${t('mapPopup.temp')}</span><br/><span style="color:#E8E6E0;">${p.temperature}°C</span></div>
        </div>
      </div>`);
    });
    ...
    map.on('click', 'rad-dots', e => {
      ...
      popup(coords, `<div style="${pStyle}border:1px solid ${color}40;">
        ...
        <div style="display:grid;grid-template-columns:1fr;gap:4px;font-size:11px;">
          <div><span style="color:#5C5A54;font-size:9px;">${t('mapPopup.reading')}</span><br/><span style="color:${color};font-weight:bold;">${p.reading} nSv/h</span></div>
          <div><span style="color:#5C5A54;font-size:9px;">${t('mapPopup.status')}</span><br/><span style="color:${color};">${p.status}</span></div>
          <div><span style="color:#5C5A54;font-size:9px;">${t('mapPopup.network')}</span><br/><span style="color:#E8E6E0;">${p.network}</span></div>
        </div>
      </div>`);
    });
    ...
    map.on('click', 'ship-dots', e => {
      ...
      popup(coords, `<div style="${pStyle}border:1px solid ${color}40;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;font-size:9px;">
          <div><span style="color:#5C5A54;">${t('mapPopup.type')}</span><br/><span style="color:${color};">${p.type.toUpperCase()}</span></div>
          <div><span style="color:#5C5A54;">${t('mapPopup.speed')}</span><br/><span style="color:#E8E6E0;">${p.speed} knots</span></div>
          <div><span style="color:#5C5A54;">${t('mapPopup.heading')}</span><br/><span style="color:#E8E6E0;">${p.heading}°</span></div>
          <div><span style="color:#5C5A54;">${t('mapPopup.destination')}</span><br/><span style="color:#E8E6E0;">${p.destination || t('common.unknown')}</span></div>
        </div>
      </div>`);
    });
    ...
    map.on('click', 'weather-dots', e => {
      ...
      popup(coords, `<div style="${pStyle}border:1px solid rgba(224,64,251,0.3);">
        <div style="color:#E040FB;font-size:14px;font-weight:700;margin-bottom:6px;">${iconEmoji} ${p.type || t('mapPopup.weatherEvent')}</div>
        <div style="font-size:10px;color:#E8E6E0;margin-bottom:8px;line-height:1.4;">${p.title || t('mapPopup.unknownEvent')}</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;font-size:9px;margin-bottom:8px;">
          <div><span style="color:#5C5A54;">${t('mapPopup.severity')}</span><br/><span style="color:${p.severity === 'high' ? '#FF1744' : '#FFD700'};">${(p.severity||'low').toUpperCase()}</span></div>
          <div><span style="color:#5C5A54;">${t('mapPopup.coords')}</span><br/><span style="color:#E8E6E0;">${coords[1].toFixed(3)}°, ${coords[0].toFixed(3)}°</span></div>
        </div>
        <div style="display:flex;gap:6px;">
          ${p.source ? `<a href="${p.source}" target="_blank" style="${linkStyle}color:#E040FB;border:1px solid rgba(224,64,251,0.4);background:rgba(224,64,251,0.1);">📡 ${t('mapPopup.source')}</a>` : ''}
          <a href="https://eonet.gsfc.nasa.gov/api/v3/events/${p.id || ''}" target="_blank" style="${linkStyle}color:var(--gold-primary);border:1px solid rgba(var(--accent-rgb),0.4);background:rgba(var(--accent-rgb),0.1);">🛰️ NASA EONET</a>
        </div>
      </div>`);
    });
    ...
    map.on('click', 'infra-dots', e => {
      ...
      popup(coords, `<div style="${pStyle}border:1px solid rgba(118,255,3,0.3);">
        <div style="color:#76FF03;font-size:14px;font-weight:700;margin-bottom:4px;">☢️ ${p.name || t('layerPanel.layers.infrastructure')}</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:9px;margin-bottom:8px;">
          <div><span style="color:#5C5A54;">${t('mapPopup.status')}</span><br/><span style="color:${statusColor};">${p.status || '—'}</span></div>
          <div><span style="color:#5C5A54;">${t('mapPopup.city')}</span><br/><span style="color:#E8E6E0;">${p.city || '—'}, ${p.country || ''}</span></div>
          <div><span style="color:#5C5A54;">${t('mapPopup.reactors')}</span><br/><span style="color:#76FF03;">${p.reactors || '—'}</span></div>
          <div><span style="color:#5C5A54;">${t('mapPopup.capacity')}</span><br/><span style="color:#E8E6E0;">${p.capacityMW ? p.capacityMW.toLocaleString() + ' MW' : '—'}</span></div>
          <div><span style="color:#5C5A54;">${t('mapPopup.owner')}</span><br/><span style="color:#E8E6E0;">${p.owner || '—'}</span></div>
          <div><span style="color:#5C5A54;">${t('mapPopup.coords')}</span><br/><span style="color:#E8E6E0;">${coords[1].toFixed(3)}°, ${coords[0].toFixed(3)}°</span></div>
        </div>
        <a href="https://www.google.com/maps/@${coords[1]},${coords[0]},14z/data=!3m1!1e3" target="_blank" style="${linkStyle}color:#76FF03;border:1px solid rgba(118,255,3,0.4);background:rgba(118,255,3,0.1);">${t('mapPopup.satelliteView')}</a>
      </div>`);
    });
    ...
    map.on('click', 'maritime-dots', e => {
      ...
      const typeLabel = p.type === 'naval' ? t('mapPopup.navalBase') : p.type === 'energy' ? t('mapPopup.energyPort') : t('mapPopup.containerPort');
      popup(coords, `<div style="${pStyle}border:1px solid ${typeColor}40;">
        ...
        ${p.volume ? `<div style="font-size:9px;color:#aaa;">${t('mapPopup.volume')}: <span style="color:${typeColor};font-weight:bold;">${p.volume}</span></div>` : ''}
        ${p.fleet ? `<div style="font-size:9px;color:#aaa;">${t('mapPopup.fleet')}: <span style="color:${typeColor};font-weight:bold;">${p.fleet}</span></div>` : ''}
        ${p.rank ? `<div style="font-size:9px;color:#aaa;">${t('mapPopup.globalRank')}: <span style="color:${typeColor};font-weight:bold;">#${p.rank}</span></div>` : ''}
      </div>`);
    });
    ...
    map.on('click', 'choke-dots', e => {
      ...
      popup(coords, `<div style="${pStyle}border:1px solid ${riskCol}40;">
        ...
        <div style="font-size:9px;color:#aaa;">${t('mapPopup.traffic')}: <span style="color:#fff;">${p.traffic}</span></div>
        <div style="font-size:9px;color:#aaa;">${t('mapPopup.risk')}: <span style="color:${riskCol};font-weight:bold;">${p.risk}</span></div>
      </div>`);
    });
  ...
  if (webglError) {
    return (
      <div className="absolute inset-0 w-full h-full flex items-center justify-center bg-[var(--bg-primary)]">
        <div className="text-center p-4 max-w-[520px] rounded-lg border border-red-900/30 bg-[var(--bg-secondary)]">
          <div className="text-xs font-mono text-red-400 tracking-widest mb-2">
            ⚠ {t('map.webglDisabledTitle')}
          </div>
          <div className="text-[11px] font-mono text-[var(--text-muted)] leading-relaxed">
            {t('map.webglDisabledBody')}
          </div>
          <div className="mt-2 text-[10px] font-mono text-[var(--text-muted)] opacity-70 break-words">
            {webglError.startsWith('webgl_') ? '' : webglError}
          </div>
        </div>
      </div>
    );
